...
plot_profile = function(act, wear = NULL, id, dates){
...
shinyServer(function(input, output) {
  ...
  observe({
    ...
    Maindata$act = read.csv(file1$datapath)
    Maindata$wear = read.csv(file2$datapath)
  })
  
  
  output$idchoices = renderUI({
    if(is.null(Maindata$act )){return()}
    selectInput("ID", label = "ID", choices = unique(Maindata$act $ID))
  })
  output$daychoices = renderUI({
    if(is.null(Maindata$act )){return()}
    selectInput("Dates", label = "Dates", choices = Maindata$act $Date,multiple = T)
  })
  ...
  output$plot <- renderPlot({
    ...
    #   wear = Maindata$wear
    # }
    plot_profile(act = Maindata$act , wear = Maindata$wear,
